    ...
    <div class="max-w-md mx-auto bg-white p-6 rounded-lg shadow-md">
      ...
      <form @submit.prevent="handleRegister">
        <div class="mb-4">
          ...
          <input v-model="email" type="email" id="email" class="w-full p-3 border rounded-lg" required />
        </div>
        <div class="mb-4">
          <label class="block text-black font-semibold mb-2" for="password">Adgangskode</label>
          <input v-model="password" type="password" id="password" class="w-full p-3 border rounded-lg" required />
        </div>
        <div class="mb-6">
          <label class="block text-black font-semibold mb-2" for="confirmPassword">Bekræft adgangskode</label>
          <input v-model="confirmPassword" type="password" id="confirmPassword" class="w-full p-3 border rounded-lg" required />
        </div>
        <button type="submit" class="w-full bg-light-green text-white font-semibold py-3 rounded-lg hover:bg-dark-green transition">
          Opret konto
        </button>
      </form>
      <p class="mt-6 text-center text-dark-brown">
        Har du allerede en konto?
        <router-link to="/login" class="text-dark-green font-semibold hover:underline">Log ind her</router-link>
      </p>
    </div>
  </section>
</template>
