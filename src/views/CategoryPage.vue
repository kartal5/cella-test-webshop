<template>
    <section class="container mt-10 mx-auto">
      <h2 class="text-3xl font-bold text-center text-navbar-green mb-8">{{ categoryTitle }}</h2>
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 gap-6">
        <div v-for="(product, index) in products" :key="index" class="bg-white p-4 rounded-lg shadow hover:shadow-lg transition-shadow flex flex-col items-center w-full">
          ...
          <p class="text-center text-gray-600 mb-2">{{ product.description }}</p>
          <p class="text-lg font-bold text-navbar-green">{{ product.price }}</p>
          <button class="bg-navbar-green text-white font-semibold py-2 px-4 rounded mt-3 w-full hover:bg-green-700 transition">Læg i kurv</button>
        </div>
      </div>
    </section>
  </template>
